...
- `evolution.router.test.ts`
  ...
  - verifies `info`, `updateConfig`, `getPayments`, `processPayments`, `monitorChest`, `monitorParties`, `updateGameStats`, and `getScene` use own-property descriptor handler resolution, deterministic missing-handler errors, and context-preserving invocation.
- `router-auth.test.ts`
