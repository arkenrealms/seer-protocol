...
- `evolution.router.test.ts`
  ...
  - verifies `info`, `updateConfig`, `getPayments`, `processPayments`, `monitorChest`, `monitorParties`, `updateGameStats`, and `getScene` resolve handlers through own-property descriptors, raise deterministic unavailable-handler errors, and preserve service context.
