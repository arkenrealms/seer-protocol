...
- `getPatrons` now checks for an own-property `Oasis.getPatrons` handler, resolves it through `Object.getOwnPropertyDescriptor(...).value`, and raises deterministic `TRPCError(INTERNAL_SERVER_ERROR)` when service wiring is unavailable or non-callable.