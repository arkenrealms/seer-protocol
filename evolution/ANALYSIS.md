...
- 2026-02-18 (rotation seer-protocol): Hardened `updateGameStats` dispatch with own-property descriptor resolution + deterministic `TRPCError(INTERNAL_SERVER_ERROR)` when `Evolution.updateGameStats` is missing/non-callable, preserving explicit failure semantics for admin stats sync paths.
- 2026-02-18 (rotation seer-protocol): Hardened `monitorChest` dispatch with own-property descriptor resolution + deterministic `TRPCError(INTERNAL_SERVER_ERROR)` when `Evolution.monitorChest` is missing/non-callable, aligning admin chest-monitor wiring with the guarded handler-invocation pattern used elsewhere in this router.
- 2026-02-18 (rotation seer-protocol): Hardened `updateConfig` dispatch with own-property descriptor resolution + deterministic `TRPCError(INTERNAL_SERVER_ERROR)` when `Evolution.updateConfig` is missing/non-callable, preventing ambiguous admin-config runtime failures in partially wired service containers.
- 2026-02-18 (rotation seer-protocol): Hardened `getPayments`/`processPayments` dispatch with own-property descriptor resolution + deterministic `TRPCError(INTERNAL_SERVER_ERROR)` when payment handlers are missing/non-callable, reducing ambiguity on financial-path service miswiring.
